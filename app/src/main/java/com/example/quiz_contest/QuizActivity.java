		...
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case 3:
					break;
				default:
						Toast.makeText(getApplicationContext(), "အဖြေမှန်တစ်ခုရွေးပေးရပါမည်။", Toast.LENGTH_SHORT).show();
						return;
				}
				if(res==Integer.parseInt(c.getString(6)))
					{
						Toast.makeText(getApplicationContext(), "မှန်တယ်။", Toast.LENGTH_SHORT).show();
						scr+=nscr;
				...
				else
				{
					...
					// mp.start();
					Toast.makeText(getApplicationContext(), "မှားတယ်။", Toast.LENGTH_SHORT).show();
				}
				if(questions<10)
				{
					//c.moveToNext();
